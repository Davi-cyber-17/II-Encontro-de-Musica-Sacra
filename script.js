...
const state = {
  ...
  myRegistration: null,
  config: { pricePerDay: 50, paymentInstructions: 'O comprovante deve estar legível e ser referente ao valor total da inscrição.', availability: {} }
};
...
async function loadEventConfig() {
  try {
    ...
    const price = Number(state.config.pricePerDay || 50);
    document.getElementById('paymentPriceText').textContent = `R$ ${price.toFixed(2).replace('.', ',')} por dia`;
...
function calculateTotal() {
  return (regData.dias || []).length * Number(state.config.pricePerDay || 50);
}
...
function updatePaymentUI() {
  ...
  document.getElementById('paymentSummaryBlock').innerHTML = `
    ...
    <div class="summary-row"><span class="k">Valor por dia</span><span class="v">R$ ${Number(state.config.pricePerDay || 50).toFixed(2).replace('.', ',')}</span></div>
    <div class="summary-row"><span class="k">Total</span><span class="v">R$ ${total.toFixed(2).replace('.', ',')}</span></div>`;
}
