  ...
  async function localApi(url, options = {}) {
    ...
    if (path === '/api/register' && method === 'POST') {
      ...
      const reg = {
        ...
        total:Number((data.dias.length * Number(config.pricePerDay || 50)).toFixed(2)),
    ...
    if (path === '/api/admin/config' && method === 'PUT') {
      ...
      if (!Number.isFinite(price) || price < 0) return jsonResponse({error:'Preço por dia inválido.'},400);
      const next = {...current,pricePerDay:price,paymentInstructions:String(p.paymentInstructions || ''),vacancies};
